...
/// setNotifications
///
export const setNotifications = async (userIds: string[], groupId: string, reset = false, remove = false) => {
    ...
    userIds.forEach(async (userId) => {
        ...
        if (remove && notifications.groups[groupId] !== undefined) {
            ...
            notifications.groups[groupId] = reset ? 0 : 
                notifications.groups[groupId] ? notifications.groups[groupId] + 1 : 1
        }
